...
export async function fetchPosts(params: FetchPostsParams): Promise<PageResponse<PostSummary>> {
...
export async function createPost(input: PostCreateInput): Promise<PostDetail> {
  ...
  const { data } = await apiClient.post<PostDetail>('/posts', buildPostFormData(input));
...
export async function updatePost(postId: number, input: PostUpdateInput): Promise<PostDetail> {
  const { data } = await apiClient.patch<PostDetail>(`/posts/${postId}`, buildPostFormData(input));
...
export async function downloadAttachment(postId: number, attachment: Attachment): Promise<void> {
  const response = await apiClient.get(`/posts/${postId}/attachments/${attachment.id}`, {
    responseType: 'blob',
  });
